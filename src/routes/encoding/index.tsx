...
const toolIcons = {
  base64: TbOutlineBinary,
  url: TbOutlineWorld,
  'html-entities': TbOutlineBraces,
  hash: TbOutlineShieldLock,
  jwt: TbOutlineKey,
  base32: TbOutlineLockSquare,
  base58: TbOutlineCoinBitcoin,
  'binary-text': TbOutlineNumbers,
  morse: TbOutlineRadio,
  caesar: TbOutlineRotate,
}

export default function EncodingIndex() {
  const category = categoryById('encoding')!
  const tools = toolsByCategory('encoding')
    ...
    <main class="w-full py-10">
      ...
      <Show when={tools.length > 0} fallback={<p class="text-sm text-muted-foreground">Tools coming soon.</p>}>
        <div class="grid gap-2 sm:grid-cols-2">
          <For each={tools}>
            {(tool) => <ToolCard tool={tool} icon={toolIcons[tool.slug as keyof typeof toolIcons] ?? TbOutlineLock} />}
          </For>
        </div>
      </Show>
    </main>
