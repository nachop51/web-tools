...
const toolIcons = {
  'color-converter': TbOutlineColorFilter,
  contrast: TbOutlineEye,
  picker: TbOutlineColorSwatch,
  'tint-shade': TbOutlineAdjustments,
  palette: TbOutlinePalette,
  gradient: TbOutlinePaint,
  'color-blindness': TbOutlineEyeOff,
  apca: TbOutlineContrast2,
  mixer: TbOutlineDroplet,
  'named-colors': TbOutlineTag,
}

export default function ColorIndex() {
  const category = categoryById('color')!
  const tools = toolsByCategory('color')
    ...
    <main class="w-full py-10">
      ...
      <Show when={tools.length > 0} fallback={<p class="text-sm text-muted-foreground">Tools coming soon.</p>}>
        <div class="grid gap-2 sm:grid-cols-2">
          <For each={tools}>
            {(tool) => (
              <ToolCard tool={tool} icon={toolIcons[tool.slug as keyof typeof toolIcons] ?? TbOutlinePalette} />
            )}
          </For>
        </div>
      </Show>
