...
import { sphereFrom, sphereInputs, type SphereInput } from '~/lib/utils/geometry/sphere'
...
const modeOptions: { value: SphereInput; label: string }[] = [
  { value: 'radius', label: 'Radius' },
  { value: 'diameter', label: 'Diameter' },
  { value: 'circumference', label: 'Circumf.' },
  { value: 'surfaceArea', label: 'Surface' },
...
export default function SphereCalculator() {
  setToolPageMeta('geometry', 'sphere')
  const [searchParams, setSearchParams] = useSearchParams<{ mode?: string; v?: string }>()

  const mode = createMemo<SphereInput>(() => {
    const m = searchParams.mode
    if (m === 'radius' || m === 'diameter' || m === 'circumference' || m === 'surfaceArea' || m === 'volume') return m
    return 'radius'
  })
  ...
  const result = createMemo(() => {
    ...
    return sphereFrom(mode(), v)
  })

  const inputLabel = createMemo(() => sphereInputs.find((c) => c.id === mode())!)
  ...
  const rows: ResultRow[] = [
    ...
    { label: 'Surface area', formula: 'A = 4πr²', value: () => fmt(result()?.surfaceArea ?? NaN) },
    { label: 'Volume', formula: 'V = ⁴⁄₃ πr³', value: () => fmt(result()?.volume ?? NaN) },
  ]
  ...
  return (
    <main class="w-full py-10">
      <ToolHeader
        ...
        description="Find radius, diameter, circumference, surface area, and volume from any one property."
      />
          ...
          <Show
            ...
            fallback={
              <div class="flex min-h-[8.25rem] items-center justify-center rounded-md border border-dashed border-border bg-muted/30 p-4 text-center text-sm text-muted-foreground">
                Enter a value to see all five properties
              </div>
            }
          >
