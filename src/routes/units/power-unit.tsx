...
const unitOptions: UnitOption[] = powerUnitKeys.map((key) => ({
  label: powerUnits[key].label,
  value: key,
}))
...
export default function PowerConverter() {
  ...
  const initialFrom = params.from && powerUnits[params.from] ? params.from : 'w'
        ...
        <section class="relative rounded-lg border border-border bg-card p-6 text-card-foreground shadow-sm transition-shadow duration-200 hover:shadow-md sm:p-8">
          ...
          >
            <div class="anim-fade-up overflow-hidden rounded-md border border-border">
              <For each={powerUnitKeys}>
                {(unit) => {
                  const converted = createMemo(() =>
                    fmt(convert(numericValue(), powerUnits[fromUnit()].factor, powerUnits[unit].factor))
                  )
