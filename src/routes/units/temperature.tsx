...
import {
  ...
  tempUnits,
  type TempUnit,
} from "~/lib/utils/units/temperature";
import { cn } from "~/lib/utils";
import { setToolPageMeta } from "~/lib/seo";

type UnitOption = { label: string; value: string };

const unitOptions: UnitOption[] = tempUnits.map((key) => ({
  label: tempUnitDefs[key].label,
  value: key,
}));
...
function isValidTempUnit(v: string | undefined): v is TempUnit {
  return v === "c" || v === "f" || v === "k";
}
...
export default function TemperatureConverter() {
  ...
  const initialFrom: TempUnit = isValidTempUnit(params.from)
    ? params.from
    : "c";
              ...
              <For each={tempUnits}>
                {(unit) => {
                  const converted = createMemo(() => {
                    ...
                    if (isNaN(n)) return "—";
                    return fmt(convertTemp(n, fromUnit(), unit));
                  });
