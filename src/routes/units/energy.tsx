...
const unitOptions: UnitOption[] = energyUnitKeys.map((key) => ({
  label: energyUnits[key].label,
  value: key,
}))
...
export default function EnergyConverter() {
  ...
  const initialFrom = params.from && energyUnits[params.from] ? params.from : 'j'
        ...
        <section class="relative rounded-lg border border-border bg-card p-6 text-card-foreground shadow-sm transition-shadow duration-200 hover:shadow-md sm:p-8">
          ...
          >
            <div class="anim-fade-up overflow-hidden rounded-md border border-border">
              <For each={energyUnitKeys}>
                {(unit) => {
                  const converted = createMemo(() =>
                    fmt(convert(numericValue(), energyUnits[fromUnit()].factor, energyUnits[unit].factor))
                  )
