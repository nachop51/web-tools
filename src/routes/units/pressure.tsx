...
const unitOptions: UnitOption[] = pressureUnitKeys.map((key) => ({
  label: pressureUnits[key].label,
  value: key,
}))
...
export default function PressureConverter() {
  ...
  const initialFrom = params.from && pressureUnits[params.from] ? params.from : 'kpa'
        ...
        <section class="relative rounded-lg border border-border bg-card p-6 text-card-foreground shadow-sm transition-shadow duration-200 hover:shadow-md sm:p-8">
          ...
          >
            <div class="anim-fade-up overflow-hidden rounded-md border border-border">
              <For each={pressureUnitKeys}>
                {(unit) => {
                  const converted = createMemo(() =>
                    fmt(convert(numericValue(), pressureUnits[fromUnit()].factor, pressureUnits[unit].factor))
                  )
