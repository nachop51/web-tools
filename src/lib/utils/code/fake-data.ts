...
const URL_PATHS = ['/', '/about', '/contact', '/blog', '/products', '/services', '/users', '/api/v1', '/docs', '/help']
...
export function generateValue(field: FieldDef): unknown {
  switch (field.type) {
    ...
    case 'date': {
      const minMs = field.minDate ? new Date(field.minDate).getTime() : new Date('2000-01-01').getTime()
      const maxMs = field.maxDate ? new Date(field.maxDate).getTime() : new Date('2030-12-31').getTime()
...
export function generateRows(fields: FieldDef[], count: number): Record<string, unknown>[] {
...
export function toSql(rows: Record<string, unknown>[], tableName: string, fields: FieldDef[]): string {
