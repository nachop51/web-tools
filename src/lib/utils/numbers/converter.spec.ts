...
import { computeOutputNumber, NumberMode, validateNumberBase } from './converter'
...
describe('validateNumberBase', () => {
  ...
  it("rejects '.' in INT mode", () => {
    expect(validateNumberBase('1.5', 10, NumberMode.INT)).toMatch(/Character "\." is not valid/)
  })
...
describe('computeOutputNumber', () => {
  ...
  it('wraps -1 to unsigned 32-bit max in INT32 mode', () => {
    expect(computeOutputNumber('-1', 10, 2, NumberMode.INT32)).toBe('11111111111111111111111111111111')
  ...
  it('emits IEEE-754 single-precision bits for FLOAT32 (decimal → binary)', () => {
    expect(computeOutputNumber('1', 10, 2, NumberMode.FLOAT32)).toBe('00111111100000000000000000000000')
  })
